...
def plot_sampling_distribution(n_points=10000):

    # Generate points from importance sampling distribution
    mu_x, sigma_x = -0.4, 0.6
    mu_y, sigma_y = 0, 0.6
    
    x_vect = np.random.normal(mu_x, sigma_x, n_points)
    y_vect = np.random.normal(mu_y, sigma_y, n_points)

    indicators = np.array([is_point_in_mandelbrot(x_vect[i], y_vect[i], 100) 
                            for i in range(n_points)])

    x_m = x_vect[indicators == 1]
    y_m = y_vect[indicators == 1]

    # Plot points
    plt.figure(figsize=(10, 10))
    plt.scatter(x_vect, y_vect, s=0.2)
    plt.scatter(x_m, y_m, s=0.2, color='red')
    plt.xlim(-2, 2)
    plt.ylim(-2, 2)
    plt.title('Importance Sampling Distribution')
    plt.grid(True, alpha=0.3)
    plt.savefig('../images/IS_sampling.pdf')